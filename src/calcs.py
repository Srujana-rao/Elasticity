...
def compute_all(
    ...
    # Step 1: MW_i for each compound
    MW_list = [molecular_weight(c.formula, atomic_weights) for c in inputs]
    coef_list = [c.coefficient for c in inputs]

    # Step 2: Correct average molecular weight for the mixture
    total_mass = sum(MW * coef for MW, coef in zip(MW_list, coef_list))
    total_moles = sum(coef_list)
    M = total_mass / total_moles if total_moles > 0 else float("nan")
    ...
    # Step 8: total packing density Vt (correct formula)
    Vt = (rho_mixture / M) * sum(Vi * Xi_i for Vi, Xi_i in zip(Vi_list, Xi)) if M > 0 else float("nan")
    ...
    E_star = 8.36 * Vt * Gt
    K_star = 10.0 * (Vt ** 2) * Gt
    S_star = (3 * K_star) / (10.2 * Vt - 1.0) if (10.2 * Vt - 1.0) != 0 else float("nan")
    L_star = K_star + (4.0 / 3.0) * S_star
    mu_star = 0.5 - 1.0 / (7.2 * Vt) if Vt != 0 else float("nan")
